app.dom = {
  ...
  async submitProfileEdit(){ // Called when the submit button is pressed on the profile edit modal
    ...
    if(res.success){
      ...
      if(altered.profile_picture[0]){  oldData['profile_picture'] = res.info['profile_picture']; $(`*[data-user-info-profile-picture="${currentUser}"]`).attr('src', 'https://socialmedia.gavhern.com/api/cdn.php?thumb&f='+res.info['profile_picture']);};
